...
import { Pencil, Trash2, Tag, AlertCircle, CheckCircle, Plus, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function ManageBrand() {
  ...
  const fetchBrands = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMsg('Failed to fetch brand list.');
      }
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async () => {
    ...
    setSuccessMsg('');
    try {
      const res = await fetch(`${API_URL}/products/brands/${deleteId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (res.ok) {
        setSuccessMsg(data.message || 'Brand deleted successfully.');
        fetchBrands();
      } else {
        setErrorMsg(data.message || 'Failed to delete brand.');
      }
    } catch (err) {
      console.error(err);
      setErrorMsg('Error communicating with the server.');
  ...
  const filteredBrands = brands.filter(b =>
    (b.BrandName || '').toLowerCase().includes(filterQuery.toLowerCase())
  );

  const totalRows = filteredBrands.length;
        ...
        <div style={{
          ...
          <div>
            ...
            <h1 style={{ fontSize: '28px', fontWeight: '800', margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
              <Tag size={28} className="text-primary" /> Manage Brands
            </h1>
          </div>

          <Link to="/admin/add-brand" className="glass-btn" style={{ borderRadius: '12px', padding: '10px 20px', fontSize: '14px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div className="glass-panel" style={{ borderRadius: '20px', padding: '24px', background: 'rgba(15, 23, 42, 0.75)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
            ...
            <span style={{ fontSize: '13px', color: 'var(--text-muted)', fontWeight: '600' }}>
              Showing {totalRows > 0 ? offset + 1 : 0} - {Math.min(offset + limit, totalRows)} of {totalRows} brand(s)
            </span>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '14px' }}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="5" style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>
                      No brands found. <Link to="/admin/add-brand" style={{ color: 'var(--primary)', fontWeight: '700' }}>Add one now</Link>.
                    </td>
                  </tr>
                ) : (
                  paginatedBrands.map((row, idx) => {
                    ...
                    return (
                      <tr key={row.ID} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.15s' }}>
                        ...
                        <td style={{ padding: '14px 18px', fontWeight: '700', color: '#fff' }}>{row.BrandName}</td>
                        ...
                        <td style={{ padding: '14px 18px', textAlign: 'right' }}>
                          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                            ...
                            <button
                              onClick={() => confirmDelete(row.ID, row.BrandName)}
                              style={{
          ...
          <div style={{
            ...
            <div className="glass-panel" style={{ width: '100%', maxWidth: '420px', padding: '28px', borderRadius: '20px', background: 'rgba(15, 23, 42, 0.95)' }}>
              ...
              <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.8)', marginBottom: '24px', lineHeight: '1.5' }}>
                Are you sure you want to permanently delete the brand <strong>"{deleteBrandName}"</strong>? This action cannot be undone.
              </p>
