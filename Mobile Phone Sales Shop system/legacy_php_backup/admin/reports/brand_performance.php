...
if ($product_name !== "") {
    $where_clauses[] = "p.ProductName = '$product_name'";
}
...
if ($product_name !== "") $all_params['product_name'] = $product_name;
...
while ($row = mysqli_fetch_assoc($kpi_res)) {
    $bname = mysqli_real_escape_string($conn, $row['ProductName']);
    ...
    $cost_q = mysqli_query($conn, "
        ...
        JOIN tblproducts p ON v.ProductId = p.ID 
        JOIN tbl_order_master m ON oi.OrderMasterId = m.ID 
        WHERE p.ProductName = '$bname' AND $where_sql
    ");
...
while ($row = mysqli_fetch_assoc($res)) {
    foreach ($brands_all as $b_all) {
        if ($b_all['ProductName'] === $row['ProductName']) {
            $brands[] = $b_all;
            break;
        }
    }
            ...
            <div class="card shadow-sm border-0 mb-4 no-print">
                <div class="card-body">
                    <form method="get" class="row g-3 align-items-end">
                        ...
                        <div class="col-md-3">
                            <label class="form-label small fw-semibold mb-1">Prodct Name</label>
                            <select name="product_name" class="form-select">
                                <option value="">All products</option>
                                <?php
                                $all_brands_q = mysqli_query($conn, "SELECT DISTINCT ProductName FROM tblproducts WHERE Status=1 ORDER BY ProductName ASC");
                                while($ab = mysqli_fetch_assoc($all_brands_q)) {
                                    $selected = ($product_name === $ab['ProductName']) ? 'selected' : '';
                                    echo "<option value='".htmlspecialchars($ab['ProductName'])."' $selected>".htmlspecialchars($ab['ProductName'])."</option>";
                                }
                                ?>
                            </select>
                        </div>
            ...
            <?php 
                ...
                foreach ($brands as $b) {
                    if ($b['return_rate'] > 5.0) {
                        $high_defect_brands[] = htmlspecialchars($b['ProductName']) . " (" . number_format($b['return_rate'], 1) . "%)";
                    }
                }
            ?>
                            ...
                            <tbody>
                                <?php if (count($brands) > 0): ?>
                                    <?php foreach ($brands as $b): ?>
                                        <tr>
                                            <td class="ps-3 fw-bold text-dark fs-6"><?= htmlspecialchars($b['ProductName']); ?></td>
...
document.addEventListener('DOMContentLoaded', function() {
    <?php
    $b_names = array_map(function($b){ return $b['ProductName']; }, $brands_all);
